...
class FinanceDashboard extends Component {
    ...
    get groupedTransactions() {
        ...
        for (const tx of rows) {
            if (!cur || cur.journal !== tx.journal) {
                if (cur) groups.push(cur);
                cur = { journal: tx.journal, items: [] };
            }
            cur.items.push(tx);
        }
        if (cur) groups.push(cur);
        ...
        for (const g of groups) {
            ...
            const sumR = g.items.reduce((s, r) => s + (parseFloat(r.received) || 0), 0);
            const sumP = g.items.reduce((s, r) => s + (parseFloat(r.paid)     || 0), 0);
            result.push({
                _type:     'subtotal',
                _journal:  g.journal,
