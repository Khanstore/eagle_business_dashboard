...
class DashboardData(models.AbstractModel):
    ...
    def get_dashboard(self, from_date=False, to_date=False):
        ...
        return {
            "quotations": [{"id": q.id, "name": q.name, "partner": q.partner_id.name or "Public User", "status": q.state, "amount": q.amount_total} for q in quotations],
            "orders": [{"id": o.id, "name": o.name, "partner": o.partner_id.name or "Public User", "partner_id": o.partner_id.id, "status": o.state, "amount": o.amount_total} for o in orders],
            "purchases": [{"id": p.id, "name": p.name, "partner": p.partner_id.name or "Supplier", "partner_id": p.partner_id.id, "status": p.state, "amount": p.amount_total} for p in purchases],
            "rfq": [{"id": r.id, "name": r.name, "partner_id": r.partner_id.id, "partner": r.partner_id.name or "Supplier", "status": r.state, "amount": r.amount_total} for r in rfq],
            "transactions": [{"id": t.id, "name": t.name or "Draft Payment", "partner_id": t.partner_id.id, "partner": t.partner_id.name or "No Partner", "ledger": t.journal_id.name, "received": t.amount if t.payment_type == 'inbound' else 0, "paid": t.amount if t.payment_type == 'outbound' else 0, "state": t.state} for t in payments]
        }
    ...
    def get_journal_balance(self, from_date=False, to_date=False):
        ...
        Return one row per bank/cash journal with:
          opening        – balance before from_date  (credit – debit on the journal's default account)
          deposit        – credits posted during the period  (money coming IN)
          deposit_count  – number of credit lines during the period
          withdraw       – debits  posted during the period  (money going OUT)
          withdraw_count – number of debit lines during the period
          closing        – opening + deposit – withdraw
        """
